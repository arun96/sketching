...
public class ReadScreenerCluster extends Screener {

  // Main function for screening reads
  // This will take in both a cluster and a screen (the original screen)
  ReadScreenerCluster(Screen sg, ClusterGenerator cg) throws Exception
  {
    ...
    if (Settings.IN_CHUNKS){
     ...
     for (int source = 0; source < numReadSets; source++){
       ...
       while (!fully_read){
         ...
         //Screen these reads
         ParallelScreenerCluster ps = new ParallelScreenerCluster(sg, reads, source, read_start, cg, cluster_map);
         ps.run();
